...
func main() {
	...
	for scanner.Scan() {
		...
		for i := 0; i < splitInstruction[0]; i++ {
			stacks[splitInstruction[2]] = append(stacks[splitInstruction[2]], stacks[splitInstruction[1]][len(stacks[splitInstruction[1]]) - 1])
			stacks[splitInstruction[1]] = stacks[splitInstruction[1]][:len(stacks[splitInstruction[1]]) - 1]
		}
	}
